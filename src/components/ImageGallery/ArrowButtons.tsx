...
import { useStore } from '@nanostores/react'
  ...
  export const PrevButton: React.FC<PropType> = (props) => {
    ...
    return (
      <button
        className={`embla__button embla__button--prev w-10 transition-colors duration-300  ${restProps.disabled ? '!text-timberwolf/25' : ''}`}
        type="button"
        {...restProps}
      >
  ...
  export const NextButton: React.FC<PropType> = (props) => {
    ...
    return (
      <button
        className={`embla__button embla__button--next w-10 transition-colors duration-300  ${restProps.disabled ? '!text-timberwolf/25' : ''}`}
        type="button"
        {...restProps}
      >
        <svg className="embla__button__svg" viewBox="0 0 532 532">
